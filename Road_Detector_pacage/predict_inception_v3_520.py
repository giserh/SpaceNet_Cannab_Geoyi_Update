...
import timeit
import cv2
from models import get_inception_v3_unet
import skimage.io
...
if __name__ == '__main__':
    ...
    for it in [0, 1]:
        ...
        for i, city in enumerate(city_datasets):
            # if i in ignored_cities or not path.isfile(path.join(models_folder, 'inception3_520_model_weights4_{0}_{1}.h5'.format(cities[i], it))):
            #     models.append(None)
            #     continue
            ...
            model = get_inception_v3_unet(input_shape, weights=None)
            model.load_weights(path.join(models_folder, 'inception3_520_model_weights4_{0}_{1}.h5'.format(cities[i], it)))
            models.append(model)
