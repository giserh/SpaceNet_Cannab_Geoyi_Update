    ...
    while True:
        ...
        for i in train_idx:
            for j in range(1):
                ...
                img = cv2.copyMakeBorder(img, 20, 20, 20, 20, cv2.BORDER_REFLECT_101)
                msk = cv2.copyMakeBorder(msk, 20, 20, 20, 20, cv2.BORDER_REFLECT_101)
...
if __name__ == '__main__':
    ...
    for all_train_idx, all_val_idx in kf.split(all_files):
        ...
        for cid, city_ in enumerate(cities):
            ...
            model = get_resnet50_linknet(input_shape)

            model.compile(loss=dice_logloss3,
                          optimizer=SGD(lr=1e-2, decay=1e-6, momentum=0.9, nesterov=True),
                          metrics=[dice_coef, dice_coef_rounded, metrics.binary_crossentropy])

            model_checkpoint = ModelCheckpoint(path.join(models_folder, 'linknet_520_model_weights_{0}_{1}.h5'.format(cities[city_id], it)), monitor='val_dice_coef_rounded',
            ...
            model.load_weights(path.join(models_folder, 'linknet_520_model_weights_{0}_{1}.h5'.format(cities[city_id], it)))
            model.compile(loss=dice_logloss,
                          optimizer=Adam(lr=1e-4),
                          metrics=[dice_coef, dice_coef_rounded, metrics.binary_crossentropy])
            model_checkpoint2 = ModelCheckpoint(path.join(models_folder, 'linknet_520_model_weights2_{0}_{1}.h5'.format(cities[city_id], it)), monitor='val_dice_coef_rounded',
                                               save_best_only=True, save_weights_only=True, mode='max')
            model.fit_generator(generator=batch_data_generator(train_idx, batch_size),
            ...
            model.load_weights(path.join(models_folder, 'linknet_520_model_weights2_{0}_{1}.h5'.format(cities[city_id], it)))
            model.compile(loss=dice_logloss2,
                          optimizer=Adam(lr=1e-5),
                          metrics=[dice_coef, dice_coef_rounded, metrics.binary_crossentropy])
            model_checkpoint3 = ModelCheckpoint(path.join(models_folder, 'linknet_520_model_weights3_{0}_{1}.h5'.format(cities[city_id], it)), monitor='val_dice_coef_rounded',
                                               save_best_only=True, save_weights_only=True, mode='max')
            model.fit_generator(generator=batch_data_generator(train_idx, batch_size),
            ...
            model.load_weights(path.join(models_folder, 'linknet_520_model_weights3_{0}_{1}.h5'.format(cities[city_id], it)))
            model.compile(loss=dice_logloss3,
                          optimizer=Adam(lr=5e-6),
                          metrics=[dice_coef, dice_coef_rounded, metrics.binary_crossentropy])
            model_checkpoint4 = ModelCheckpoint(path.join(models_folder, 'linknet_520_model_weights4_{0}_{1}.h5'.format(cities[city_id], it)), monitor='val_dice_coef_rounded',
                                               save_best_only=True, save_weights_only=True, mode='max')
            model.fit_generator(generator=batch_data_generator(train_idx, batch_size),
